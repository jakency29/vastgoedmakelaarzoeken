...
// contact en RealEstateAgent structured data. Leadformulier centraal via Elfsight.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!k) return {};
  return {
    title: { absolute: `${k.naam}: vastgoedkantoor in ${k.gemeente}`.slice(0, 60) },
    description: `${k.naam} is een erkend vastgoedkantoor in ${k.gemeente} (${k.provincie}). Bekijk de diensten en het werkingsgebied en vraag vrijblijvend een offerte.`.slice(0, 155),
          ...
          <div className="min-w-0">
            {k.foto && (
              <div className="mb-6 flex items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 p-6">
                ...
                <img src={k.foto} alt={k.naam} width={900} height={600} className="max-h-64 w-auto object-contain" />
              </div>
            )}
            ...
            <div className="mt-8 rounded-2xl border border-brand-200 bg-brand-50 p-5">
              <p className="text-sm text-brand-900">
                Wil je dit kantoor of een ander kantoor in je buurt vergelijken? Vraag rechts
                vrijblijvend een offerte aan. Wij bezorgen je aanvraag aan erkende vastgoedmakelaars.
              </p>
            </div>
          </div>
